                ...
                link: function (scope,element,attr,controller) {
                    ...
                    angular.element(element).ready(function() {
                        ...
                        if (oImage.complete) {
                            //$timeout.cancel();
                            //$timeout(function(){
                            scope.$emit("waterfall:repeatFinished",element.parent());
                            //},500);
                        }else {
        ...
        .directive("ngWaterfall",function($window,$document,$rootScope,$timeout,$log){
            return {
                ...
                link: function(scope,element){
                    ...
                    $rootScope.$on("waterfall:repeatFinished",function(e,ele){
                        $timeout.cancel();
                        $timeout(function(){
                            waterfall(scope.minCols,ele);
                        },500)
                    });
                    ...
                    function waterfall(minCols,ele){
                        if(ele){
                            element=ele;
                            var c=element.attr("cols");
                            if(c)
                                minCols=c;
                        }
