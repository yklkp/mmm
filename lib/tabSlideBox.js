...
.directive('onFinishRender', function ($timeout, $ionicSlideBoxDelegate) {
    return {
        restrict: 'A',
...
.directive('tabSlideBox', [ '$timeout', '$window', '$ionicSlideBoxDelegate', '$ionicScrollDelegate',
		...
		return {
			...
			link : function(scope, element, attrs, ngModel) {
				...
				function renderScrollableTabs(){
					...
					var initialIndex = parseInt(attrs.tab);
			...
			controller : function($scope, $attrs, $element) {
				...
				$scope.slideHasChanged = function(item){
					$scope.events.trigger("slideChange", {"index" : item});
					$timeout(function(){if($scope.onSlideMove) $scope.onSlideMove({"index" : eval(item)});},100);
				};
	...
	.directive('onTagFinishRender', function ($timeout, $ionicSlideBoxDelegate) {
		return {
			restrict: 'A',
			link: function (scope, element, attr) {
				$timeout(function () {
...
//只实现点击标签时滑动居中
.directive('tagSlideBox', [ '$timeout', '$window', '$ionicScrollDelegate',
	function($timeout, $window, $ionicScrollDelegate) {
		...
		return {
			...
			link : function(scope, element, attrs, ngModel) {
				...
				var ta = element[0], $ta = element;
				$ta.addClass("tabbed-slidebox");
				var handle = ta.querySelector('.scroll').getAttribute('delegate-handle');
				var ionicScrollDelegate = $ionicScrollDelegate;
				if(handle){
					ionicScrollDelegate = ionicScrollDelegate.$getByHandle(handle);
				}
